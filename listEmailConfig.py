...
try:
    ...
    for info in EmailConfig:
        ...
        if info.emailType == "TAX_CHECK":
            print(
                "[정발행] %s(공급자에게 전자세금계산서 수신확인 메일 전송 여부) : %s"
                % (info.emailType, info.sendYN)
            )
        if info.emailType == "TAX_CANCEL_ISSUE":
            print(
                "[정발행] %s(공급받는자에게 전자세금계산서 발행취소 메일 전송 여부) : %s"
                % (info.emailType, info.sendYN)
            )
        if info.emailType == "TAX_REQUEST":
            print(
                "[역발행] %s(공급자에게 세금계산서를 발행요청 메일 전송 여부) : %s"
                % (info.emailType, info.sendYN)
            )
        if info.emailType == "TAX_CANCEL_REQUEST":
            print(
                "[역발행] %s(공급받는자에게 세금계산서 취소 메일 전송 여부) : %s"
                % (info.emailType, info.sendYN)
            )
        ...
        if info.emailType == "TAX_TRUST_ISSUE_TRUSTEE":
            print(
                "[위수탁발행] %s(수탁자에게 전자세금계산서 발행 메일 전송 여부) : %s"
                % (info.emailType, info.sendYN)
            )
        if info.emailType == "TAX_TRUST_ISSUE_INVOICER":
            print(
                "[위수탁발행] %s(공급자에게 전자세금계산서 발행 메일 전송 여부) : %s"
                % (info.emailType, info.sendYN)
            )
        if info.emailType == "TAX_TRUST_CANCEL_ISSUE":
            print(
                "[위수탁발행] %s(공급받는자에게 전자세금계산서 발행취소 메일 전송 여부) : %s"
                % (info.emailType, info.sendYN)
            )
        if info.emailType == "TAX_TRUST_CANCEL_ISSUE_INVOICER":
            print(
                "[위수탁발행] %s(공급자에게 전자세금계산서 발행취소 메일 전송 여부) : %s"
                % (info.emailType, info.sendYN)
            )
        ...
        if info.emailType == "TAX_NTSFAIL_INVOICER":
            print(
                "[처리결과] %s(전자세금계산서 국세청 전송실패 안내 메일 전송 여부) : %s"
                % (info.emailType, info.sendYN)
            )
        if info.emailType == "ETC_CERT_EXPIRATION":
            print(
                "[정기발송] %s(팝빌에서 이용중인 공인인증서의 갱신 메일 전송 여부) : %s"
                % (info.emailType, info.sendYN)
            )
